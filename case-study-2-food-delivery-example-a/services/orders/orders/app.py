...
def create_delivery_order():
    ...
    return rpc_auth_create(
        "POST", {"order_id": order_counter, "order_amount": order_amount}
    )
...
def create_takeout_order():
    ...
    return rpc_takeout_auth_create(
        "POST", {"order_id": order_counter, "order_amount": order_amount}
    )
...
def update_takeout_order(order_id):
    ...
    return rpc_takeout_auth_update("PUT", order_details)
...
def delete_takeout_order(order_id):
    ...
    return rpc_takeout_auth_delete("DELETE", order_details)


@circuit
def rpc_auth_delete(verb, order_details):
    return rpc_auth(verb, "delivery_auth", order_details)
...
@circuit
def rpc_takeout_auth_delete(verb, order_details):
    return rpc_auth(verb, "takeout_auth", order_details)


@circuit
def rpc_takeout_auth_update(verb, order_details):
    return rpc_auth(verb, "takeout_auth", order_details)


@circuit
def rpc_takeout_auth_create(verb, order_details):
    return rpc_auth(verb, "takeout_auth", order_details)
...
def rpc_auth(verb, order_type, order_details):
    ...
    try:
        ...
        if order_type == "delivery_auth":
            response = requests.request(
                verb,
                "{}/delivery_auth".format(helper.get_service_url("delivery_auth")),
                timeout=helper.get_timeout("delivery_auth"),
                json={"order_details": order_details},
            )
        # here we are communicating with the takeout_auth service
        if order_type == "takeout_auth":
            response = requests.request(
                verb,
                "{}/takeout_auth".format(helper.get_service_url("takeout_auth")),
                timeout=helper.get_timeout("takeout_auth"),
                json={"order_details": order_details},
            )

...
@app.route("/auth/<order_type>", methods=["DELETE"])
def auth_delete_takeout(order_type):

    if str(order_type) == "delivery":
...
@app.route("/auth/<order_type>", methods=["PUT"])
def auth_put_takeout(order_type):
    if str(order_type) == "delivery" or str(order_type) == "takeout":
        return jsonify(request.json), 200
    else:
        return jsonify(request.json), 403
...
@app.route("/auth/<order_type>", methods=["POST"])
def auth_post_takeout(order_type):
    if str(order_type) == "delivery" or str(order_type) == "takeout":

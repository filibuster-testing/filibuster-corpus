...
def test_cinema_10_showtimes_index():
    client = app.test_client()
    reply = client.get("/")
    actual_reply = reply.json
    assert reply.status_code == 200

def test_cinema_10_booking_showtimes_showtimes():
    client = app.test_client()
    reply = client.get("/showtimes")
    actual_reply = reply.json
    assert len(actual_reply) == 6

def test_cinema_10_booking_showtimes_showtime():
    client = app.test_client()
...
def test_cinema_10_booking_showtimes_invalid_showtime():
    client = app.test_client()

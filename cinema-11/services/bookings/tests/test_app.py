...
def test_cinema_11_booking_records_index():
    client = app.test_client()
    reply = client.get("/")
    actual_reply = reply.json
    assert reply.status_code == 200

def test_cinema_11_booking_records_users():
    client = app.test_client()
    reply = client.get("/bookings")
    actual_reply = reply.json
    assert len(actual_reply) == 3

def test_cinema_11_booking_records_user():
    client = app.test_client()
...
def test_cinema_11_booking_records_unknown_user():
    client = app.test_client()

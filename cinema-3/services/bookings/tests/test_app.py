...
def test_cinema_3_booking_records_index():
    client = app.test_client()
    reply = client.get("/")
    actual_reply = reply.json
    assert reply.status_code == 200

def test_cinema_3_booking_records_users():
    client = app.test_client()
...
def test_cinema_3_booking_records_user_success(mock_get):
    client = app.test_client()
...
def test_cinema_3_booking_records_user_fail_movies(mock_get):
    client = app.test_client()
    response = client.get("/bookings/{}".format("chris_rivers"))
...
def test_cinema_3_booking_records_user_timeout_movies(mock_get):
    client = app.test_client()
    response = client.get("/bookings/{}".format("chris_rivers"))
    assert response.get_data() == b'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">\n<title>503 Service Unavailable</title>\n<h1>Service Unavailable</h1>\n<p>The Movie service timed out.</p>\n'

def test_cinema_3_booking_records_unknown_user():
    client = app.test_client()

...
with open("{}/cinema-4/services/bookings/bookings.json".format(os.path.dirname(parent_path)), "r") as f:
    bookings = json.load(f)

with open("{}/cinema-4/services/movies/movies.json".format(os.path.dirname(parent_path)), "r") as f:
    movies = json.load(f)
...
def test_cinema_4_user_index():
    client = app.test_client()
    reply = client.get("/")
    assert reply.status_code == 200

def test_cinema_4_user_users():
    client = app.test_client()
    reply = client.get("/users")
    actual_reply = reply.json
    assert len(actual_reply) == 7

def test_cinema_4_user_user():
    client = app.test_client()
...
def test_cinema_4_user_invalid_user():
    client = app.test_client()
...
def test_cinema_4_user_user_suggested():
    client = app.test_client()
    reply = client.get("/users/{}/suggested".format("chris_rivers"))
    actual_reply = reply.json
    assert actual_reply == None

def test_cinema_4_user_user_invalid_bookings():
    client = app.test_client()
...
def test_cinema_4_user_user_bookings_success(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
    assert response.get_data() == b'{"20151201":["267eedb8-0f5d-42d5-8f43-72426b9fb3e6"]}\n'
    
@mock.patch('requests.get', side_effect=mock_requests_get_with_failure_setting(MockFailure.FAIL_BOOKINGS))
def test_cinema_4_user_user_bookings_fail_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_4_user_user_bookings_timeout_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_4_user_user_bookings_no_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))

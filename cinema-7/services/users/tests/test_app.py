...
with open("{}/cinema-7/services/bookings-primary/bookings.json".format(os.path.dirname(parent_path)), "r") as f:
    bookings-primary = json.load(f)

with open("{}/cinema-7/services/bookings-secondary/bookings.json".format(os.path.dirname(parent_path)), "r") as f:
    bookings-secondary = json.load(f)

with open("{}/cinema-7/services/movies/movies.json".format(os.path.dirname(parent_path)), "r") as f:
    movies = json.load(f)
...
def mock_requests_get_with_failure_setting(failure_setting):
  ...
  def mock_requests_get(*args, **kwargs):
    health_check_request = "http://{}:{}/health-check".format(helper.resolve_requests_host('bookings-primary'), helper.get_port('bookings-primary'))
    if (args == (health_check_request,)):
...
def test_cinema_7_user_index():
    client = app.test_client()
    reply = client.get("/")
    assert reply.status_code == 200

def test_cinema_7_user_users():
    client = app.test_client()
    reply = client.get("/users")
    actual_reply = reply.json
    assert len(actual_reply) == 7

def test_cinema_7_user_user():
    client = app.test_client()
...
def test_cinema_7_user_invalid_user():
    client = app.test_client()
...
def test_cinema_7_user_user_suggested():
    client = app.test_client()
    reply = client.get("/users/{}/suggested".format("chris_rivers"))
    actual_reply = reply.json
    assert actual_reply == None

def test_cinema_7_user_user_invalid_bookings():
    client = app.test_client()
...
def test_cinema_7_user_user_bookings_success(mock_get):
    client = app.test_client()
...
def test_cinema_7_user_user_bookings_fail_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_7_user_user_bookings_timeout_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
    assert response.get_data() == b'{"20151201":[{"rating":6.4,"title":"Victor Frankenstein","uri":"/movies/7daf7208-be4d-4944-a3ae-c1c2f516f3e6"}]}\n'

@mock.patch('requests.get', side_effect=mock_requests_get_with_failure_setting(MockFailure.NO_BOOKINGS_FOUND))
def test_cinema_7_user_user_bookings_no_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_7_user_user_bookings_fail_bookings_and_fail_bookings2(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_7_user_user_bookings_fail_bookings_and_bookings2_timeout(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_7_user_user_bookings_fail_bookings_and_bookings2_not_found(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
    assert response.get_data() == b'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 3.2 Final//EN">\n<title>404 Not Found</title>\n<h1>Not Found</h1>\n<p>No bookings were found for chris_rivers</p>\n'

@mock.patch('requests.get', side_effect=mock_requests_get_with_failure_setting(MockFailure.FAIL_MOVIES))
def test_cinema_7_user_user_bookings_fail_bookings_movies(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
...
def test_cinema_7_user_user_bookings_timeout_movies(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))

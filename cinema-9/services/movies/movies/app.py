...
## TODO: remove
## send movies information to Redis upon startup
r = redis.Redis(host="localhost", port=helper.get_redis_port("movies"), decode_responses=True)
...
def movie_record():
    ## TODO: should not get all the keys of a database in a production environment
    keys = r.keys("*")
    movies = {}
    for key in keys:
        movies[key] = r.hgetall(key)

...
def test_cinema_9_user_index():
    client = app.test_client()
    reply = client.get("/")
    assert reply.status_code == 200

def test_cinema_9_user_users():
    client = app.test_client()
    reply = client.get("/users")
    actual_reply = reply.json
    assert len(actual_reply) == 7

def test_cinema_9_user_user():
    client = app.test_client()
...
def test_cinema_9_user_invalid_user():
    client = app.test_client()
...
def test_cinema_9_user_user_suggested():
    client = app.test_client()
    reply = client.get("/users/{}/suggested".format("chris_rivers"))
    actual_reply = reply.json
    assert actual_reply == None

def test_cinema_9_user_user_invalid_bookings():
    client = app.test_client()
...
def test_cinema_9_user_user_bookings_success(mock_get):
    client = app.test_client()
...
def test_cinema_9_user_user_bookings_fail_bookings(mock_get):
    client = app.test_client()
    response = client.get("/users/{}/bookings".format("chris_rivers"))
